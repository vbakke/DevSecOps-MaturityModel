...
const GRAFANA_FARO_BASEURL = 'https://faro-collector-prod-eu-north-0.grafana.net/collect/';
...
export async function onRequest(context: CloudflareContext): Promise<Response> {
  ...
  if (request.method === 'GET') {
    return new Response('GET method is not allowed. Please use POST to send logs.', {
      status: 405,
    });
  }
  ...
  const GRAFANA_FARO_URL = GRAFANA_FARO_BASEURL + env.GRAFANA_FARO_KEY;
